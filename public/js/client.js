...
/* socket io package ends here  */

/* hark package starts here */
...
(function (e) {
  if ("function" == typeof bootstrap) bootstrap("hark", e);
    ...
    "undefined" != typeof window ? (window.hark = e()) : (global.hark = e());
})(function () {
...
// this script will be loadded when the game is loaded via "UnityInitVoiceChatLogic" function in the jslib file
...
class VoiceChat {
  constructor() {
    ...
    // setup variables
    this.io = io; // the socket object that is used for communication
    this.hark = null;
    this.isMute = false; // the mute status of the current client
    this.rooms = []; // contains the rooms the client is at (can join multiple rooms at the same time)
    this.muteList = []; // will contain all the client ids that the client don't want to listen to
    this.timeInterval = 850; // the time in ms for the chunk length before sending it (best results range from 850 to 1000, the bigger the time, the bigger the delay, the smaller the time, the more stutter can be heared)
    this.tmpClientsInRoom = []; // this is a temporary variable for storing the clients id of a requested specfic room and is only relevant when the function is called
    this.UnityInstance = window.unityInstance; // this will be set by exposing the unity instance after it's creation in the window variable to enable access in this class

    // will make sure that the url has been assigned from the unity game
    var url =
    ...
    this.MicrohphoneManager.on("streamAvailable", (stream) => {
      console.log("stream is available from the microphone manager", stream);
      this.InitHark(stream);
      self.RecordAudio(stream);
    });

    this.io.on("connect", () => {
      console.log(self.io.id);
    });

    this.io.on("Rooms", (rooms) => {
      self.rooms = rooms;
      // update the unity instance
      if (self.UnityInstance != null || self.UnityInstance != undefined) {
        var json = { "rooms": rooms };
        self.UnityInstance.SendMessage(
          "VoiceChatHandler",
          "SetRooms",
          JSON.stringify(json)
        );
    ...
    this.io.on("UPDATE_VOICE", function (data, senderID) {
      if (!self.IsOtherClientMuted(senderID)) {
        var audio = new Audio(data);
        audio.play();
      }
    });

    this.io.on("clientsInRoom", function (clients) {
      self.tmpClientsInRoom = clients;

      // set the client list in unity
      if (self.UnityInstance != null || self.UnityInstance != undefined) {
        var json = { "clients": clients };
        console.log(json);
        self.UnityInstance.SendMessage(
          "VoiceChatHandler",
          "SetClientsInRoom",
    ...
    this.io.on("onJoinRoom", function (clientID, roomID) {
      console.log("the client " + clientID + " has joined room " + roomID);
      // TODO: update the list
      self.tmpClientsInRoom.push(clientID);
      if (self.UnityInstance != null || self.UnityInstance != undefined) {
        var json = { "clients": self.tmpClientsInRoom };
        console.log(json);
        self.UnityInstance.SendMessage(
          "VoiceChatHandler",
          "SetClientsInRoom",
    ...
    this.io.on("onLeftRoom", function (clientID, roomID) {
      console.log("the client " + clientID + " has Left room " + roomID);
      const index = self.tmpClientsInRoom.indexOf(clientID);
      self.tmpClientsInRoom.splice(index, 1);
      // TODO: update the list
      if (self.UnityInstance != null || self.UnityInstance != undefined) {
        var json = {"clients": self.tmpClientsInRoom}
        console.log(json);
  ...
  /**
   * will take an array buffer (base64 audio) and emit it to the socket server
   * the socket server have a list of the rooms the player is in
   * @param {ArrayBuffer} audioArrayBuffer
   */

  EmitVoice(audioArrayBuffer) {
    if (!this.isMute) {
      this.io.emit("VOICE", audioArrayBuffer);
    }
  }

  /**
   * handles recording the audio
   *
   * it works as an endless recurrsion loop in the mediarcorder variable
  ...
  /**
   * intializes hark and registers events for when the client is or isn't speaking by the provided audio stream
   *
   * @param {MediaStream} stream
   */
  InitHark(stream) {
    // to use hark (package that allows the detection of speaking threshold)
    var options = {
      threshold: -80,
    };
    this.hark = window.hark(stream, options);

    this.hark.on("speaking", function () {
      console.log("speaking");
      // TODO: call unity function
      if (self.UnityInstance != null || self.UnityInstance != undefined) {
        self.UnityInstance.SendMessage(
          "VoiceChatHandler",
          "SetIsSpeaking",
    ...
    this.hark.on("stopped_speaking", function () {
    ...
    this.hark.on("volume_change", function (data) {});
  }

  /**
   * returns the state of whether is the client is speaking or not
   * @returns {boolean}
   */
  IsSpeaking() {
    // make sure that there is an audio stream otherwise there is no point of checking
    if (
      this.MicrohphoneManager.localStream == null ||
      this.MicrohphoneManager.localStream == "" ||
      this.MicrohphoneManager.localStream == undefined
    ) {
      return false;
    } else {
      return this.hark.speaking;
    }
  }

  /**
   * returns the current client socket id
   * @returns {string}
   */
  GetMySocketID() {
    return this.io.id;
  ...
  /**
   * checks if an other client is locally muted by the current/local client
   * @param {string} clientID
   * @returns {boolean}
   */
  IsOtherClientMuted(clientID) {
    var isMuted = false;
    this.muteList.forEach((muteListClientID) => {
      if (muteListClientID == clientID) {
        isMuted = true;
      }
    });
    return isMuted;
  ...
  /**
   * adds another client id to the mute list
   * @param {string} clientID
   */
  MuteClient(clientID) {
    this.muteList.push(clientID);
  }

  /**
   * searches for a client id to be removed from the mute list if there is one
   * @param {string} clientID
   */
  UnmuteClient(clientID) {
    var index = this.muteList.indexOf(clientID);
    if (index !== -1) {
  ...
  /**
   * unmute all of the locally muted clients for the current client
   */
  UnmuteAllClients() {
    this.muteList = [];
  }
  ...
  /**
   * handles fetching the rooms the client has joined in
   * @returns {Array}
   */
  GetRooms() {
    // response is handled in "Rooms" event in the constructor
    this.io.emit("getRooms");
    return this.rooms;
  }

  GetClientsInRoom(roomID) {
    this.io.emit("getClientsInRoom", roomID);
  }

  /**
   * handles joining the client into a room
   * @param {string} roomID
   */
  JoinRoom(roomID) {
    this.io.emit("joinRoom", roomID);
    this.rooms = this.GetRooms();
    this.GetClientsInRoom(roomID);
  }

  /**
   * handles leaving the client from a room
   * @param {string} roomID
   */
  LeaveRoom(roomID) {
  ...
  /**
   * leave all the rooms the player is in
   */
  LeaveAllRooms() {
    this.io.emit("leaveAllRooms");
    this.rooms = this.GetRooms();
  }
}

class Microhphone extends EventEmitter{
  constructor(voiceChat) {
    super();
    this.voiceChat = voiceChat;
    var self = this;
    this.localStream = null;
    this.microhphonesList = null;
    this.isPermited = false;

    this.GetLocalStream();
    this.GetAvailableMicrophones()
      .then((microphones) => {
        self.microhphonesList = microphones;    
    ...
    await navigator.permissions
      ...
      .then((permissionStatus) => {
        if (permissionStatus.state === "granted") {
          ...
          console.log("Permission to access microphone is granted");
          return self.isPermited;
        } else {
          self.isPermited = false;
          console.log("Permission to access microphone is not granted");
          return self.isPermited;
        }
      })
      .catch((error) => {
      ...
      if (self.UnityInstance != null || self.UnityInstance != undefined) {
        var json = { "canAccess": self.isPermited };
        self.voiceChat.UnityInstance.SendMessage(
          "VoiceChatHandler",
  ...
  async GetAvailableMicrophones() {
    ...
    if (self.voiceChat.UnityInstance != null || self.voiceChat.UnityInstance != undefined) {
      var json = { "microphones": microphones };
    ...
    console.log(JSON.stringify({ "microphones": microphones }));
    return microphones;
  }

  // Create a function to handle microphone selection change
  HandleMicrophoneChange(stream, microphoneId) {
    ...
    navigator.mediaDevices
      ...
      .then((newStream) => {
        // Do something with the new microphone stream
        console.log("Microphone access granted!");
        self.localStream = newStream;
        self.emit("streamAvailable", newStream);
        // You can use the `newStream` object to process the audio stream
      })
      .catch((error) => {
  ...
  GetLocalStream() {
    var self = this;
    // get the client audio stream
    navigator.mediaDevices
      .getUserMedia({ audio: true })
      .then((stream) => {
        self.localStream = stream;
        self.emit("streamAvailable", stream);
      })
      .catch((err) => {
...
class ChatManager extends EventEmitter{
  constructor(voiceChat){
    ...
    var self = this;
    this.voiceChat = voiceChat;
    this.io = this.voiceChat.io;

    this.io.on("UpdateChat", function (message, clientID) {
      self.OnUpdateChatHandler(message, clientID);
    });
  ...
  OnUpdateChatHandler(message, clientID){
    ...
    var jsonString = JSON.stringify({"clientID": clientID, "message": message})
    self.voiceChat.UnityInstance.SendMessage(
      "VoiceChatHandler",
      "ReceiveChatMessage",
      jsonString
...
var voiceChat;
console.log("socket io has loaded");
voiceChat = new VoiceChat();
// to save a global refrence to the instance of the voice chat
